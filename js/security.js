...
exports = module.exports = function (pluginConfig, sriConfig) {
  ...
  const memoizeCacheThatExpiresAfterAWhile = new ExpiryMap(5 * 60 * 1000); // cache for 5 minutes
  ...
  const rawUrlToKeySet = async (rawUrlStr, tx) => {
    ...
    try {
      ...
      debug(
        "sri-security",
        "security db check, securitydb_time=" + (new Date() - start) + " ms."
      );
  ...
  const checkKeysAgainstDatabase = async (relevantSriRequests) => {
    ...
    await pMap(
      ...
      async (keyStr) => {
        ...
        if (
          ability === "read" &&
          pluginConfig.securityDbCheckMethod === "CacheRawListResults"
        ) {
          ...
          debug(
            "sri4node-security-api | security db check, securitydb_time=" +
              (new Date() - start) +
              " ms."
          );
        }
